    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h3 class="page-title">Glue Return</h3>
            </div>
        </div>
    </div>
    <!-- end page title -->
...
<?php endif; ?>
    ...
    <div class="row">
        ...
        <div class="col-6">
            <form action="<?php echo e(route('glue_return.reports')); ?>" method="GET" class="form-inline" id="search-form">
                <div class="row">
    ...
    <select name="account_id" class="form-control select2" data-toggle="select2">
        ...
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
               ...
               <div class="form-group mt-3">
                                    ...
                                    <a class="btn btn-success" href="<?php echo e(route('glue_return.list')); ?>" role="button" onclick="return checkPermission()">Add New</a>
                                </div>
                </div>
            </form>
        </div>
            ...
            <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo e($data->gluereturns->items->item_code ?? 'N/A'); ?></td>
                        <td><?php echo e($data->gluereturns->qty ?? 'N/A'); ?></td>
                        <td><?php echo e($data->gluereturns->rate ?? 'N/A'); ?></td>
                        <td><?php echo e($data->gluereturns->amount ?? 'N/A'); ?></td>
                        <td class="no-print">
                                                <input type="checkbox"
                        ...
                        <td class="no-print">
            <?php if($data->gluereturns): ?> <!-- Check if plate return exists -->
            <form action="<?php echo e(route('glue_return.destroy', $data->gluereturns->id)); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?');">
                <?php echo csrf_field(); ?>
...
function printTable() {
 ...
 const freightContent = `
   <?php
    ...
    $freightDetails = $trndtl->pluck('gluereturns')->filter(function ($detail) {
        return $detail->freight > 0;
    });

    // Calculate total quantities
    $totalQtys = $trndtl->pluck('gluereturns')->where('vorcher_no')->sum('qty');
?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\probox\resources\views/glue_return/index.blade.php ENDPATH**/ ?>