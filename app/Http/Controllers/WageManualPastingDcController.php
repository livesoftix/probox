...
use App\Models\WageManualPastingDc;
...
class WageManualPastingDcController extends Controller
{
...
$usedVouchers = WageManualPastingDc::select('v_no', 'dc_type')
    ->distinct()
    ->get()
    ->map(function ($row) {
...
->filter(function ($dc) {
    ...
    if (!$product || $product->manual_pasting != 1) {
        return false;
    }

    // Same DC + same product already used?
    $alreadyUsed = WageManualPastingDc::where('dc_type', 'pharma')
        ->where('v_no', $dc->v_no)
...
->unique(function ($dc) {
    return $dc['type'] . '-' . $dc['v_no'] . '-' . $dc['prod_id'];
})
->values();
...
->filter(function ($dc) {
    ...
    $alreadyUsed = WageManualPastingDc::where('dc_type', 'confectionery')
        ->where('v_no', $dc->v_no)
        ->where('prod_id', $prodId)
        ->exists();
    ...
    if ($type === 'pharma') {
        ...
        foreach ($masters as $master) {
            ...
            */
            ...
            $clabour = $product->manual_pasting_rate ?? 0;
...
public function store(Request $request)
{
    ...
    DB::transaction(function () use ($request) {
        ...
        foreach ($entries as $entry) {
            ...
            $wage = WageManualPastingDc::create([
                ...
                'manualpasting_wage' => $entry['manualpasting_wage'],
                'total_amount' => $totalAmount,
                'dc_type' => $entry['dc_type'],
                'date' => $request->date,
                'prepared_by' => $request->prepared_by,
                'v_type' => 'Manual Pasting DC',
            ]);
...
if ($request->has('employee_id')) {
    ...
    foreach ($request->employee_id as $index => $employeeId) {
        ...
        TRNDTL::create([
            ...
            'date'        => $request->date,
            'description' => 'Wage Manual Pasting DC Deduction ',
...
$contractorAmount = $totalAmount - $totalDeduction;
        TRNDTL::create([
            'v_no' => $newInvoiceNumber,
            'date' => $request->date,
            'description' => 'Manual Pasting DC Wage',
            'account_id' => $acc_id,
            'cash_id' => null,
            'preparedby' => auth()->user()->name,
    ...
    return redirect()
        ->route('manualPasting_wage_dc.report')
        ->with('success', 'Manual Pasting DC wage stored successfully.');
}
...
public function edit($b_no)
{
    ...
    $voucher = WageManualPastingDc::where('b_no', $b_no)->firstOrFail();

    // DC rows only
    // dd($b_no);
    $entries = WageManualPastingDc::where('b_no', $b_no)
        ->whereNotNull('v_no')
        ->where('v_type','Manual Pasting DC')
        ->get();
        // dd($entries);

    // Employee rows only
    $employeeRows = WageManualPastingDc::where('b_no', $b_no)
...
$usedVouchers = WageManualPastingDc::where('b_no', '!=', $b_no)
    ->select('v_no', 'dc_type')
    ->distinct()
    ->get()
    ->map(function ($row) {
...
public function update(Request $request, $b_no)
{
    ...
    DB::transaction(function () use ($request, $b_no) {
        ...
        TRNDTL::where('v_no',$b_no)
            ->where('v_type','Wage Manual Pasting DC')
            ->delete();
        ...
        foreach($entries as $entry){
            ...
            $row=WageManualPastingDc::create([
                'b_no'=>$b_no,
                'v_no'=>$entry['v_no'],
                'dc_date'=>$entry['dc_date'],
                'account_id'=>$entry['account_id'],
                ...
                'qty'=>$entry['qty'],
                'clabour'=>$entry['clabour'],
                'manualpasting_wage'=>$entry['breaking_wage'],
                'total_amount'=>$totalAmount,
                'dc_type'=>$entry['dc_type'],
                'date'=>$request->date,
                'prepared_by'=>$request->prepared_by,
...
public function report(Request $request)
{
    ...
    $query = WageManualPastingDc::query();
    ...
    */
    ...
    $WageManualPastings->each(function ($item) {

        $item->total_deduction = WageManualPastingDc::where(
            'b_no',
            $item->b_no
        )->sum('deduction');
        ...
        /*
        |--------------------------------------------------------------------------
        | Get Contractor For This Specific Voucher
        |--------------------------------------------------------------------------
        |
        | IMPORTANT:
        | We do NOT query EmployeeType here.
        |
        | The contractor account was already saved in TRNDTL
        | when the voucher was created.
        |
        */

        $item->contractor = TRNDTL::with('accounts')
            ->where('v_no', $item->b_no)
            ->where('v_type', 'Wage Manual Pasting DC')
            ->where('description', 'Manual Pasting DC Wage')
            ->where('debit', 0)
            ->where('credit', '>', 0)
            ->orderByDesc('id')
            ->first();
    ...
    */
    ...
    $vNoList = WageManualPastingDc::select(
            'b_no',
            'dc_type'
        )
        ->distinct()
  ...
  public function destroy($id)
{
    try {
        ...
        DB::transaction(function () use ($id) {
            ...
            $wageCorrugationDc = WageManualPastingDc::findOrFail($id);

            $bNo = $wageCorrugationDc->b_no;

            // Delete all DC wage rows of same voucher
            WageManualPastingDc::where('b_no', $bNo)->delete();
        ...
        return redirect()
            ->route('manualPasting_wage_dc.report')
            ->with('success', 'Manual Pasting DC wage entries deleted successfully.');
        ...
        return redirect()
            ...
            ->with(
                'error',
                'Error deleting Manual Pasting DC wage entries: ' . $e->getMessage()
            );
    }
}
...
{
    $voucher = WageManualPastingDc::where('b_no', $b_no)->firstOrFail();
    $products = WageManualPastingDc::select(
            'prod_id',
            'product_name',
            'clabour',
            ...
            DB::raw('SUM(manualpasting_wage) as amount')
        )
        ->where('b_no', $b_no)
    ...
    $employees = WageManualPastingDc::with('employee')
        ->where('b_no', $b_no)
        ->whereNotNull('employee_id')
        ->get();

    return view(
        'wages.manual_pasting.print',
        compact(
            'voucher',
            'products',
            'employees',
