...
class ShipperReturnController extends Controller
{
  public function index()
  {
    ...
    return view('shipper_return.list', get_defined_vars());
  }
  public function store(Request $request)
  {
    ...
    $lastEntry = ShipperReturn::orderBy('id', 'desc')->first();
    $newInvoiceNumber = $lastEntry ? ((int) $lastEntry->vorcher_no + 1) : 1;
    foreach ($request->entries as $index => $entry) {
      $itemCode = DB::table('item_masters')
        ->where('id', $entry['item'] ?? 0)
        ->value('item_code');

      $purchaseGlue = ShipperReturn::create([
    ...
    return redirect()->route('shipper_return.reports')->with('success', 'Voucher No. ' . $newInvoiceNumber . ' has been saved successfully.');
  }
  ...
  public function reports(Request $request)
  {
    ...
    $query = TRNDTL::where('v_type', 'Shipper-Return')->where('credit', 0)->where('account_id', '!=', 35)->with('shipperreturns');
    ...
    $vNo = TRNDTL::where('v_type', 'Shipper-Return')->pluck('v_no')->unique()->toArray();
    $accountId = AccountMaster::whereIn('id', TRNDTL::where('v_type', 'Shipper-Return')->pluck('account_id'))
      ->where('title', '!=', 'Purchase Freight')
      ->pluck('title', 'id');

    return view('shipper_return.index', [
      'trndtl' => $trndtl,
      'startDate' => $startDate,
      'endDate' => $endDate,
      'status' => $status,
   ...
   public function destroy($id)
  {
    try {
      ...
      $shipperReturn = ShipperReturn::findOrFail($id);

      TRNDTL::where('v_type', 'Shipper-Return')
        ->where('r_id', $shipperReturn->id)
        ->delete();
      ...
      return redirect()->route('shipper_return.reports')
        ->with('success', 'Shipper Return record deleted successfully.');
    } catch (\Exception $e) {
      DB::rollBack();
      return redirect()->route('shipper_return.reports')
