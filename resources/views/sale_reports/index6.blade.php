...
@section('content')
                            ...
                            <form action="{{ route('pharma_billing.reports') }}" method="GET" class="form-inline"
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-xl-2">
                                        ...
                                        <select name="item" class="form-control select2" data-toggle="select2">
                                            ...
                                            @foreach ($itemList as $itemId => $itemTitle)
                                                <option value="{{ $itemId }}"
                                                    {{ request()->get('item') == $itemId ? 'selected' : '' }}>
                                                    {{ $itemTitle ?? 'N/A' }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="account_id" class="form-control select2" data-toggle="select2">
                                            ...
                                            @foreach ($accountList as $accountId => $accountTitle)
                                                <option value="{{ $accountId }}"
                                                    {{ request()->get('account_id') == $accountId ? 'selected' : '' }}>
                                                    {{ $accountTitle ?? 'N/A' }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group col-xl-2">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="show_prev_balance"
                                                ...
                                                {{ request()->get('show_prev_balance', '1') == '1' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="showPrevYes">Yes</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="show_prev_balance"
                                                id="showPrevNo" value="0"
                                                {{ request()->get('show_prev_balance', '1') == '0' ? 'checked' : '' }}>
                                    ...
                                    <div class="form-group mt-3">
                                        ...
                                        <a class="btn btn-success" href="{{ route('pharma_billing.list') }}" role="button"
                                            onclick="return checkPermission()">Add
                                            New</a>
                                    </div>
                                </div>
                            </form>
                            ...
                            <div class="tab-content">
                                <div class="col-12">
                                        ...
                                        <tbody>
                                            @foreach ($trnDetails as $data1)
                                                ...
                                                @if (count($uniqueItemTitles) > 0)
                                                    <tr>
                                                        <td>{{ \Carbon\Carbon::parse($data1->date)->format('d-m-Y') ?? 'N/A' }}
                                                        </td>
                                                        <td>{{ strtoupper(substr($data1->accounts->title ?? 'N/A', 0, 1)) . strtoupper(substr(explode(' ', $data1->accounts->title ?? 'N/A')[1] ?? '', 0, 1)) }}-{{ $data1->v_no }}
                                                        </td>
                                                        ...
                                                        <td>
                                                            <form
                                                                action="{{ route('pharma_billing.destroy', ['billing_no' => $data1->r_id]) }}"
                                    ...
                                    <table id="combined-data-table-2"
                                        ...
                                        <h1 id="print-header" style="display: none;">
                                            {{ $status == 'official' ? 'Prinitng Cell' : 'Rizwan Packages' }}</h1>
                                        @php
                                        ...
                                        @foreach ($trnDetails as $data1)
                                            ...
                                            <div
                                                ...
                                                <h4 id="print-header1" style="margin: 0; display: none;">Printing Date:
                                                    {{ \Carbon\Carbon::parse($data1->created_at)->format('d-m-Y') ?? 'N/A' }}
                                                </h4>
                                                @if ($vNoList)
                                                    <h4 id="print-header3"
                                                        style="margin: 0; margin-right: 10px; text-align: centre; display: none; padding-top: 20px; margin-bottom: 10px;">
                                                        Invoice #: {{ $vNoList }}</h4>
                                                @endif
                                            </div>
                                        @endforeach
                                        ...
                                        @if ($accountTitles)
                                            <h1 id="print-header2" style="display: none;">Name: {{ $accountTitles }}</h1>
                                        @endif
                                        ...
                                        <tbody>
                                            ...
                                            @if ($selectedAccountId)
                                                <p style="display: none;">Selected Account ID: {{ $selectedAccountId }}
                                                </p>
                                            @endif
                                            ...
                                            @foreach ($filteredInvoices as $index => $data)
                                                @php
                                                    ...
                                                    // Compute voucher amount as sum of total * rate for this voucher number
                                                    $gtComputed = $filteredInvoices
                                                        ->where('v_no', $data->v_no)
                                                        ->reduce(function ($carry, $inv) {
                                                            $rate = isset($inv->rate) ? (float) $inv->rate : 0;
                                                            $total = isset($inv->total) ? (float) $inv->total : 0;
                                                            return $carry + $rate * $total;
                                                        }, 0.0);
                                                    $pb = $trnDetail->pre_bal ?? 0;
                                                @endphp
                                                ...
                                                @if ($acc !== 'N/A')
                                                    ...
                                                    <tr class="default-row">
                                                        <td>{{ \Carbon\Carbon::parse($data->created_at)->format('d-m-Y') ?? 'N/A' }}
                                                        </td>
                                                        <td>{{ $data->old_vno }}</td>
                                                        ...
                                                        <td style="display: none;">
                                                            {{ $data->itemType->type_title ?? 'N/A' }}</td>
                                                    ...
                                                    <tr class="official-row" style="display: none;">
                                                        ...
                                                        <td>{{ number_format($data->rate ?? 0, 2) }}</td>
                                                        <td>{{ number_format($amount, 2) }}</td>
                                                        <td>{{ $data->st_rate ?? 0 }}%</td>
                                                        <td>{{ number_format($data->st_amount ?? 0, 2) }}</td>
                                                        <td>{{ number_format($amount + ($data->st_amount ?? 0), 2) }}</td>
                                                        <td style="display: none;">
                                                            {{ $data->itemType->type_title ?? 'N/A' }}</td>
                                                        <td style="display: none;">{{ $acc }}</td>
                                                        <td style="display: none;">{{ $data->account_id ?? 'N/A' }}</td>
                                                ...
                                                @if ($index === $filteredInvoices->count() - 1 || $filteredInvoices[$index + 1]->v_no !== $data->v_no)
                                                    <tr class="default-row">
                                                        <td colspan="6" style="text-align: right; font-weight: bold;">
                                                            Grand Total: </td>
                                                        <td colspan="2" style="font-weight: bold;">
                                                            {{ number_format($gtComputed, 2) }}</td>
                                                    </tr>
                                                    <tr class="official-row" style="display: none;">
                                                        <td colspan="6" style="text-align: right; font-weight: bold;">
                                                            Total S.Tax Amount:</td>
                                                        <td style="font-weight: bold;">
                                                            {{ number_format($gtComputed, 2) }}
                                                        </td>
                                                    </tr>
                                                    {{-- <tr>
                                                        <td colspan="4" style="text-align: right; font-weight: bold;">
                                                            Amount of this Bill: </td>
                                                        <td colspan="6" style="font-weight: bold;">
                                                            {{ number_format($gtComputed, 2) }}</td>
                                                    </tr>
                                                    @if (isset($showPrevBalance) && $showPrevBalance == '1')
                                                        <tr>
                                                            <td colspan="4"
                                                                style="text-align: right; font-weight: bold;">Previous
                                                                Balance: </td>
                                                            <td colspan="6" style="font-weight: bold;">
                                                                {{ number_format($pb, 2) }}</td>
                                                        </tr>
                                                        <tr>
                                                            <td colspan="4"
                                                                style="text-align: right; font-weight: bold;">Total: </td>
                                                            <td colspan="6" style="font-weight: bold;">
                                                                {{ number_format($gtComputed + $pb, 2) }}</td>
                                                        </tr>
                                                    @else
                                                        <tr>
                                                            <td colspan="4"
    ...
    <script>
        ...
        function printTable() {
            ...
            // Build paginated table HTML: 9 rows for first page, 14 rows for subsequent pages
            ...
            if (rowsToPrint.length === 0) {
                const tfootHtml = `<tfoot><tr><td colspan="${selectedHeading === 'ProBox Packages official' ? '7' : '9'}" style="height: 50px; border: none;"></td></tr></tfoot>`;
                paginatedTableHtml = `<table style="width:100%; border-collapse: collapse;">${headerHtml}<tbody><tr><td colspan="${selectedHeading === 'ProBox Packages official' ? '7' : '9'}">No records found</td></tr></tbody>${tfootHtml}</table>`;
            } else {
