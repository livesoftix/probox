...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            @endif
            <div class="col-12">
                <a href="{{ route('process.list') }}"><button type="button" class="btn btn-primary" onclick="return checkPermission()">Add
                        Level2</button></a>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane show active" id="basic-datatable-preview">
                                <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                                    ...
                                    <tbody>
                                        @foreach ($processSections as $process)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $process->department->name ?? 'N/A' }}</td>
                                                <td>{{ $process->name ?? 'N/A'  }}</td>
                                               
                                                <td class="no-print"><form action="{{ route('process.destroy', $process->id) }}" method="POST" style="display: inline;">
            @csrf
            @method('DELETE')
