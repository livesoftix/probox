                            ...
                            <div class="tab-pane show active" id="input-types-preview">
                                <div class="row">
                                    <form id="voucherForm" action="{{ route('cash.update', $voucher->first()->v_no) }}"
                                        ...
                                        <input type="hidden" name="invoice_number" value="{{ $voucher->first()->v_no }}">
                                        ...
                                        <h3 class="mt-4">Invoice <span
                                                id="invoiceDisplay">{{ $voucher->first()->v_no }}</span></h3>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <input type="date"
                                                                        ...
                                                                        class="form-control" value="{{ $entry->date }}">
                                                                </td>
                                                                <td>
                                                                    <select name="entries[{{ $entry->id }}][cash]"
                                                                        class="form-control select2">
                                                                        <option value="">Select</option>
                                                                        @foreach ($accountMasters as $account)
                                                                            <option value="{{ $account->id }}"
                                                                                @if ($entry->cash_id == $account->id) selected @endif>
                                                                                {{ $account->title }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </td>
                                                                <td>
                                                                    <select name="entries[{{ $entry->id }}][account]"
                                                                        class="form-control select2">
                                                                        <option value="">Select</option>
                                                                        @foreach ($accounts as $account)
                                                                            <option value="{{ $account->id }}"
                                                                                @if ($entry->account_id == $account->id) selected @endif>
                                                                                {{ $account->title }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </td>
                                                                ...
                                                                <td>
                                                                    <input type="number"
                                                                        name="entries[{{ $entry->id }}][credit]"
                                                                        class="form-control" value="{{ $entry->credit }}">
                                                                </td>
                                                                <td>
                                                                    <button type="button"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addEntryButton.addEventListener('click', function() {
        ...
        newRow.innerHTML = `
            <td></td>
            <td><input type="date" class="form-control" name="entries[${entryKey}][date]" value="{{ date('Y-m-d') }}"></td>
            <td>
