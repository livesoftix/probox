            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Item Log</li>
                    </ol>
                </div>
                <h3 class="page-title">Item Registration Log</h3>
            </div>
        </div>
    </div>
    <!-- end page title -->
        ...
        <div class="card mt-2">
            <div class="card-body">
                <div class="tab-content">
                    <div class="col-12">
                        <form action="{{ route('inventory.item_log') }}" method="GET" class="form-inline"
                            id="search-form">
                            <div class="row">
        ...
        value="{{ request()->get('end_date', '') }}">
                ...
                <select id="item_code" class="form-control" name="item_code">
                    ...
                    @foreach ($items as $item)
                        <option value="{{ $item }}" 
                            {{ request()->get('item_code') == $item ? 'selected' : '' }}>
                            {{ $item }}
                        </option>
                    @endforeach
                </select>
                        ...
                        <div class="tab-content">
                            <div class="col-12">
                                ...
                                <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                                    <h4>Item Registration Log Details</h4>
                                    <thead>
                                        <tr>
                                            <th>Sr</th>
                                            <th>Date</th>
                ...
                @foreach($itemLogs as $log)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                         <td>{{ \Carbon\Carbon::parse($log->updated_at)->format('d/m/Y h:i A') }}</td>
                        <td>{{ $log->item_code }}</td>
                        <td>{{ $log->itemtypes->type_title }}</td>
                        <td>{{ $log->old_purchase }}</td>
                        <td>{{ $log->new_purchase }}</td>
                    </tr>
                @endforeach
            </tbody>
                                </table>
