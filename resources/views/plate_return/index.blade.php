...
@endif
    ...
    <div class="row">
        ...
        <div class="col-6">
            <form action="{{ route('plate_return.reports') }}" method="GET" class="form-inline" id="search-form">
                <div class="row">
    ...
    <select name="account_id" class="form-control select2" data-toggle="select2">
        ...
        @endforeach
                ...
                <div class="form-group mt-3">
                                    ...
                                    <a class="btn btn-success" href="{{ route('plate_return.list') }}" role="button" onclick="return checkPermission()">Add New</a>
                                </div>
                </div>
            </form>
        </div>
    ...
    <tr>
        ...
        <td>{{ $data->platereturns->items->item_code ?? 'N/A'}}</td>
        <td>{{ $data->platereturns->countries->country_name ?? 'N/A'}}</td>
        <td>{{ $data->platereturns->products->prod_name ?? 'N/A'}}</td>
        <td>{{ $data->platereturns->description ?? 'N/A'}}</td>
        <td>{{ $data->platereturns->qty ?? 'N/A'}}</td>
        <td>{{ $data->platereturns->rate ?? 'N/A'}}</td>
        <td>{{ $data->platereturns->amount ?? 'N/A'}}</td>
        <td class="no-print">
            <input type="checkbox"
        ...
        <td class="no-print">
            @if($data->platereturns) <!-- Check if plate return exists -->
            <form action="{{ route('plate_return.destroy', $data->platereturns->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?');">
                @csrf
                @method('DELETE')
