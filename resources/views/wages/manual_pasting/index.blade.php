...
<div class="container-fluid">
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Manual Pasting Wage DC Department</li>
                    </ol>
                </div>
                <h3 class="page-title">Manual Pasting Delivery Challan Department</h3>
            </div>
        </div>
    </div>
    <!-- end page title -->
        ...
        <div class="card mt-2">
            <div class="card-body">
                <div class="tab-content">
                    <div class="col-12">
                        <form action="{{ route('manualPasting_wage_dc.report') }}" method="GET" class="form-inline" id="search-form">
   <div class="row">
    ...
    <div class="form-group col-xl-3">
        ...
        <select name="v_no" class="form-control select2">
            ...
            @foreach($vNoList as $voucher)
                <option
                    value="{{ $voucher->dc_type }}|{{ $voucher->v_no }}"
                    {{ request('v_no') == $voucher->dc_type.'|'.$voucher->v_no ? 'selected' : '' }}>
    ...
    <div class="form-group mt-3">
        ...
        <a href="{{ route('manualPasting_wage_dc.list') }}"
                        ...
                        <div class="tab-content">
                            <div class="col-12">
                                <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                                    <h3>Manual Pasting Department Details</h3>
                                    <thead>
                                        <tr>
                                            <th>Date</th>
                                            <th>Manual Pasting DC</th>
                                            <th>Contractor</th>
                                            <th>Amount</th>
                                            <th>Action</th>
                                        </tr>
  ...
  @foreach ($WageManualPastings as $general)

    @php
        $entryKey = $general->b_no . '-' .
                    $general->employee_name . '-' .
    ...
    <tr>
        ...
        <td>
            {{ \Carbon\Carbon::parse($general->updated_at)->format('Y-m-d') }}
        </td>

        <td>
            {{ $general->v_type ?? 'N/A' }}-{{ $general->b_no ?? 'N/A' }}
        </td>

        <td>
            {{ $general->contractor?->accounts?->title ?? 'N/A' }}
        </td>

        <td>
            {{ $general->total_amount - $general->total_deduction }}
        </td>

        <td class="no-print">

            <a href="{{ route('manualPasting_wage_dc.edit', $general->b_no) }}"
               class="btn btn-warning btn-sm">
                Edit
            </a>

            <a href="{{ route('manualPasting_wage_dc.print', $general->b_no) }}"
               target="_blank"
               class="btn btn-primary btn-sm">
                Print
            </a>

            <form action="{{ route('manualPasting_wage_dc.destroy', $general->id) }}"
                  method="POST"
  ...
  function checkPermission() {
        ...
        if ($isAdmin == 0) {
            $userRights = \App\Models\Right::where('user_id', auth()->user()->id)
                ->where('app_name', 'manualPastingwages')
                ->first();
