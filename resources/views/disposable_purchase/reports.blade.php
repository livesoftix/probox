            ...
            <div class="card mt-2">
                <div class="card-body">
                    <div class="tab-content">
                        <div class="col-6">
                            <form action="{{ route('disposable_purchase.reports') }}" method="GET" class="form-inline"
                                id="search-form">
                                <div class="row">
                                    ...
                                    <div class="form-group col-xl-4">
                                        ...
                                        <select name="status" class="form-control select2">
                                            ...
                                            <option value="official"
                                                {{ request()->get('status') == 'official' ? 'selected' : '' }}>Official
                                            </option>
                                            <option value="unofficial"
                                                {{ request()->get('status') == 'unofficial' ? 'selected' : '' }}>Unofficial
                                            </option>
                                        </select>
                                    </div>
                                        ...
                                        <select name="account_id" class="form-control select2" data-toggle="select2">
                                            ...
                                            @foreach ($accountMasters as $account)
                                                <option value="{{ $account->id }}"
                                                    {{ request()->get('account_id') == $account->id ? 'selected' : '' }}>
                                                    {{ $account->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group mt-3">
                                        ...
                                        <a class="btn btn-success" href="{{ route('disposable_purchase.list') }}"
                                    ...
                                    <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                                        ...
                                        <tbody>
                                            @php
                                                ...
                                                $totalAmount = 0;
                                            @endphp
                                            @foreach ($trndtlEntries as $entry)
                                                @php
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="no-print">{{ $entry->v_type }}-{{ $entry->v_no }}</td>
                                                    <td>{{ optional($accountMasters->firstWhere('id', $entry->account_id))->title ?? 'N/A' }}
                                                    </td>
                                                    <td>{{ $entry->disposablepurchase->item->item_code ?? 'N/A' }}</td>
                                                    <td>{{ $entry->disposablepurchase->qty ?? 'N/A' }}</td>
                                                    <td>{{ $entry->disposablepurchase->weight_type ?? 'N/A' }}</td>
                                                    <td>{{ $entry->disposablepurchase->rate ?? 'N/A' }}</td>


                                                    <td>{{ $entry->disposablepurchase->amount ?? 'N/A' }}</td>
                                                    <td>{{ $entry->disposablepurchase->freight ?? 'N/A' }}</td>
                                                    <td>
                                                        @if ($entry->disposablepurchase && $entry->disposablepurchase->image)
                                                            <a href="{{ asset('storage/' . $entry->disposablepurchase->image) }}"
                                                                target="_blank">IMG</a>
                                                        @else
                                                            N/A
                                                        @endif
                                                    ...
                                                    <td class="no-print">
                                                        <input type="checkbox" class="status-checkbox"
                                                            data-id="{{ $entry->id }}"
                                                            {{ $entry->status == 'official' ? 'checked' : '' }}>
                                                    </td>
                                                    <td class="no-print">
                                                        <div class="d-flex align-items-center gap-1">
                                                            <form
                                                                action="{{ route('disposable_purchase.delete', ['id' => $entry->id]) }}"
                                                            ...
                                                            <a href="{{ route('disposable_purchase.edit', ['v_no' => $entry->v_no]) }}"
                                                                class="btn btn-warning btn-sm"
                                                                onclick="return checkPermissionEdit()"><i
                                                                    class="uil uil-edit"></i></a>
                                                            <a href="{{ route('disposable_purchase.editFreight', ['v_no' => $entry->v_no]) }}"
        ...
        function printTable() {
            ...
            if (h5Element) {
                ...
                <span style="flex: 1; font-size: 14px;">
                    Date: {{ $trndtlEntries->isNotEmpty() ? \Carbon\Carbon::parse($trndtlEntries->first()->date)->format('d-m-Y') : 'N/A' }}
                </span>
                <span style="margin-left: auto; font-size: 14px;">
                  Voucher No: {{ $trndtlEntries->isNotEmpty() ? $trndtlEntries->first()->v_type : 'N/A' }}-{{ $trndtlEntries->isNotEmpty() ? $trndtlEntries->first()->v_no : 'N/A' }}
                </span>
            `;
