                    ...
                    @forelse($dies as $index => $die)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                {{ $die->repair_count>0? $die->repair_count>0 : '—' }}
                                </span>
...
<button type="button"
        ...
        title="Edit"
        data-id="{{ $die->id }}"
        data-die-code="{{ $die->die_code }}"
        data-product-id="{{ $die->product_id }}"
        data-rate="{{ $die->rate }}"
        data-type="{{ $die->type }}"
        data-repeat-date="{{ $die->repeat_date }}"
        data-description="{{ $die->description }}"
        data-repair-count="{{ $die->repair_count ?? 0 }}"
 ...
 <button type="button"
        ...
        title="Repeat"
        onclick="openRepeatDieModal({{ $die->id }})">
...
<button type="button"
        ...
        title="Repair"
        data-id="{{ $die->id }}"
        data-item_name="{{ $die->product?->items?->item_code }}"
        data-length="{{ $die->length }}"
        data-width="{{ $die->width }}"
        data-repair-count="{{ $die->repair_count ?? 0 }}"
    ...
    <i class="fas fa-screwdriver-wrench"></i>
                                    ...
                                    {{-- Delete --}}
                                    <form action="{{ route('dies.destroy', $die) }}"
                                          method="POST"
                        ...
                        <select name="product_id"
                            ...
                            @foreach($products as $product)
                                ...
                                <option value="{{ $product->id }}"
                                        data-itemId="{{ $product->item_id }}"
                                        data-item="{{ $product->items?->item_code }}"
                                        data-length="{{ $product->length }}"
                                        data-width="{{ $product->width }}"
                                        data-ups="{{ $product->ups }}">
...
<div class="die-modal"
    ...
    <div class="die-modal-card repair-modal-card">
        ...
        <form method="POST"
              id="repairDieForm"
              action="{{ route('dies.repair.store') }}">
    ...
    <h4>
        Repair History
            ...
            <div class="die-repair-history"
                ...
                {{-- Javascript will load history here --}}
...
<div id="dieRepeatHistory"
    ...
    {{-- Javascript will load repeat history here --}}
...
window.viewDie = function (id) {
    ...
    // Build URL
    // ------------------------------------------------------------
    let url = "{{ route('dies.view.data', ':id') }}";
...
function repairDie(id) {
    ...
    fetch("{{ url('/probox/dies') }}/" + id + "/repair-data")
        .then(response => {
...
window.repairDie = function (button) {
    ...
    */
    ...
    fetch(
        "{{ url('/probox/dies') }}/" + id + "/repair-data"
    )
    .then(response => {
